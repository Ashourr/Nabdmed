...
import login from "../../imges/Frame 1597883457.png";
...
function Signup(prpos) {
      ...
      try {
        if (flog) {
          let res = await axios.post(
            "https://dashboard.nabdmed.com/api/register",
            {
              first_name: fname,
              last_name: lname,
              mobile: phone,
  ...
  function gatKey(e) {
    fetch(`https://dashboard.nabdmed.com/api/countries`)
      .then((res) => res.json())
        ...
        <div className="row">
          ...
          <div className="col-12 col-md-6 form">
              ...
              <div className="links-div">
                ...
                <div>
                  <Link to={"/signup"} className="link2">
                    {prpos.langs === "ar" ? "Register" : "انشاء حساب"}
                  </Link>
                </div>
              </div>
            </div>
            <form onSubmit={formSubmitsign}>
              ...
              <label>{prpos.langs==="ar"?"Confirm Password":"تاكيد كلمة المرور"}</label>
              <input
                className="password"
                type="password"
                placeholder={
