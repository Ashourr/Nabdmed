...
import { faChevronLeft, faLeftLong, faPen } from "@fortawesome/free-solid-svg-icons";
...
function Profile(prpos) {
  ...
  async function dataUser() {
    ...
    try {
      let res = await fetch(`https://dashboard.nabdmed.com/api/user-show`, {
        method: "GET",
        headers: {
        ...
        <div className="container ">
          {prpos.langs === "en" ? (
            <Link onClick={goback} className="back">
              <FontAwesomeIcon icon={faChevronLeft} className="iconar" /> عوده
            </Link>
          ) : (
