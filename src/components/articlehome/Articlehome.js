...
function Articlehome(prpos) {
  ...
  useEffect(() => {
    fetch("https://dashboard.nabdmed.com/api/blogger")
      .then((res) => res.json())
      ...
      <div className={`servicediv ${prpos.langs}`}  key={itme.slug}>
        <div className="service">
          <div className="ser-atim">
            ...
            {prpos.langs === "ar" ? (
              <p>{itme.title.substring(0, 40)}...</p>
            ):(
              <p>{itme.title_ar.substring(0, 40)}...</p>
            )}
            <Link  to={`/blogpage/${itme.slug}`}  className={prpos.langs}>
              {prpos.langs === "en" ? "اعرف المزيد":"Learn More"} <FontAwesomeIcon icon={faRightLong} className="icon" />
            </Link>
          </div>
        </div>
      </div>
      ...
      <div className={`articlehome ${prpos.langs}`}>
        <div className="title">
          {prpos.langs === "ar" ? (
            ...
            <h2>
              المقالات والتحديثات <span> لخبراء</span> الصحة
            </h2>
          )}
          <div>
