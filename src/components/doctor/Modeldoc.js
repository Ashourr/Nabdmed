...
function Modeldoc(props) {
  ...
  useEffect(() => {
    fetch(`https://dashboard.nabdmed.com/api/doctors-review/${id}`)
      .then((res) => res.json())
  ...
  async function formSubmit(e) {
    ...
    if(window.localStorage.getItem("email")){
      ...
      let res = await axios.post(
        "https://dashboard.nabdmed.com/api/add-doctor-rating",
        {
          message: message,
          rating: userRating,
          doctor_id: id,
        ...
        return (
          <div key={index} className="div-itme">
            <div className="div-info">
              <div className="img">
                {item.coveruser ? (
                  <img height={"60px"} src={item.coveruser} alt="User Cover" />
                ) : (
                  <img
                  height={"50px"} src={imglogo}
                    alt="Default Cover"
                  />
                )}
              </div>
                  ...
                  <div className="div-addreview">
                    <form onSubmit={formSubmit}>
                      ...
                      {error.status === 422 || error.status === 401 ? (
                        <span className="errormodel">{props.langs === "en" ? "يجب عليك تسجيل الدخول" : "You must be logged in"}</span>
                      ) : (
                        success === true &&
                        accept === true && (
