...
import { faChevronLeft, faLeftLong, faPlay } from "@fortawesome/free-solid-svg-icons";
...
function Blogone(props) {
  ...
  useEffect(() => {
    fetch(`https://dashboard.nabdmed.com/api/blogger-Show/${bolgslug}`)
      .then((res) => res.json())
      .then((data) => {
  ...
  return (
    <div className={`blogone ${props.langs}`}>
      ...
      <Link className="backblog" onClick={goback}>
          <FontAwesomeIcon icon={faChevronLeft} className={`back ${props.langs}`} /> {props.langs === "en" ? "رجوع" : "Back"}
        </Link>
